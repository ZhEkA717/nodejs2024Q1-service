...
import { FavoriteEntityType, FavoriteResponce } from './entities/favorite.entities';
...
export class FavoritesService {
    ...
    async findAll(): Promise<FavoriteResponce> {
        ...
        return {
            artists: await this.prisma.artist.findMany({ where: { id: { in: artists } } }),
            albums: await this.prisma.album.findMany({ where: { id: { in: albums } } }),
            tracks: await this.prisma.track.findMany({ where: { id: { in: tracks } } })
        }
    }

    async addEntity(id: string, entityType: FavoriteEntityType) {
        const { prismaEntity, field } = entityType;
        const entity = await this.prisma[prismaEntity].findUnique({ where: { id } })
