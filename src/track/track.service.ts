...
export class TrackService {
  ...
  async create(createTrackDto: CreateTrackDto): Promise<Track> {
    if (!(await this.validateDto(createTrackDto))) throw new BadRequestException();
    const id = v4();
  ...
  async update(id: string, updateTrackDto: UpdateTrackDto):Promise<Track> {
    ...
    const track = await this.db.findUnique({where: {id}});
    if (!track) throw new NotFoundException();
    if (!(await this.validateDto(updateTrackDto))) throw new BadRequestException();
  ...
  async validateDto(dto: CreateTrackDto | UpdateTrackDto): Promise<boolean> {
    ...
    const artistIsExist = artistId === null || await this.prisma.artist.findUnique({where: {id: String(artistId)}});
    const albumIsExist = albumId === null || await this.prisma.album.findUnique({where: {id: String(albumId)}});
